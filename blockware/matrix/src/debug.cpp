
// void debug_alphabet()
// {
//   int charsPerLine = screen.x / alphabet_textSize.x;

//   // 0 thru 3 corresponding to 4 cardinal rotations
//   // canvas->setRotation(3);
//   canvas->setTextSize(1);
//   canvas->setTextColor(NEON_GREEN);

//   Serial.printf("\n\n====================================\n\n");

//   for (uint8_t i = 0; i < alphabet_length; i++) {
//     const char* text = alphabet[i];

//     int mx = i % charsPerLine;
//     int ln = i / charsPerLine;
//     int x = 0 + (mx * alphabet_textSize.x);
//     int y = 0 + (ln * alphabet_textSize.y);

//     // Example
//     // [38] mx=8,ln=3 -> (96, 42)
//     // [39] mx=9,ln=3 -> (108, 42)
//     // [40] mx=0,ln=4 -> (0, 56)
//     // [41] mx=1,ln=4 -> (12, 56)
//     // [42] mx=2,ln=4 -> (24, 56)
//     // // print just debug information (no text measurement)
//     // Serial.printf("\n[%s] i=%d,mx=%d,ln=%d -> (%d, %d)", text, i, mx, ln, x,
//     // y); optionally print text measurements
//     int16_t text_x, text_y;
//     uint16_t text_width, text_height;
//     canvas->getTextBounds(text, 0, 0, &text_x, &text_y, &text_width, &text_height);
//     Serial.printf("\n[%s] i=%d,mx=%d,ln=%d -> %dx%d (%d, %d)", text, i, mx, ln,
//                   text_width, text_height, x, y);

//     canvas->setCursor(x, y);
//     canvas->printUTF8((char*)text);
//   }
// }

// void debug_hiragana()
// {
//   int charsPerLine = screen.x / hiragana_textSize.x;

//   // 0 thru 3 corresponding to 4 cardinal rotations
//   // canvas->setRotation(3);
//   canvas->setTextSize(1);
//   canvas->setTextColor(NEON_GREEN);

//   Serial.printf("\n\n====================================\n\n");

//   for (uint8_t i = 0; i < hiragana_length; i++) {
//     const char* text = hiragana[i];

//     int mx = i % charsPerLine;
//     int ln = i / charsPerLine;
//     int x = 0 + (mx * hiragana_textSize.x);
//     int y = 0 + (ln * hiragana_textSize.y);

//     // Example
//     // [38] mx=8,ln=3 -> (96, 42)
//     // [39] mx=9,ln=3 -> (108, 42)
//     // [40] mx=0,ln=4 -> (0, 56)
//     // [41] mx=1,ln=4 -> (12, 56)
//     // [42] mx=2,ln=4 -> (24, 56)
//     // // print just debug information (no text measurement)
//     // Serial.printf("\n[%s] i=%d,mx=%d,ln=%d -> (%d, %d)", text, i, mx, ln, x, y);

//     // Example
//     // [Z] i=25,mx=5,ln=2 -> 8x16 (60, 28)
//     // [ア] i=26,mx=6,ln=2 -> 24x16 (72, 28)
//     // optionally print text measurements
//     int16_t text_x, text_y;
//     uint16_t text_width, text_height;
//     canvas->getTextBounds(text, 0, 0, &text_x, &text_y, &text_width, &text_height);
//     Serial.printf("\n[%s] i=%d,mx=%d,ln=%d -> %dx%d (%d, %d)", text, i, mx, ln,
//                   text_width, text_height, x, y);

//     canvas->setCursor(x, y);
//     canvas->printUTF8((char*)text);
//   }
// }
