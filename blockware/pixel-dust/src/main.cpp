...
void loop() {
  ...
  int16_t axes[3];
  int ret = acc->Get_X_AxesRaw(axes);
  //Serial.printf("%d get axes returned %d, %d %d %d\n", begin_retval, ret, axes[0], axes[1], axes[2]);
  ...
  //uint8_t acc_id;
  /*
  if (acc->ReadID(&acc_id) != 0)
  {
    Serial.println("error reading id");
  }
